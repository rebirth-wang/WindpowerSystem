...
@Service
public class AiHybridQueryServiceImpl implements IAiHybridQueryService {

    private static final String QUERY_TYPE_CURRENT = "CURRENT";
    ...
    @Override
    public AiHybridQueryResultVO query(String question) {
        ...
        if (!isHybridCompatibleRoute(route.getQueryMode())) {
            throw new ServiceException(message("ai.hybrid.query.route.required"));
        }
        ...
        AiQueryGovernanceSnapshotVO governanceSnapshot =
                aiQueryGovernanceService.validateMultiSourceAccess(deviceMetaData, identifier, AiQueryMode.HYBRID_PIPELINE.name());
        QueryWindow window = resolveWindow(question);
        String queryType = resolveQueryType(question);

        AiHybridQueryResultVO result = new AiHybridQueryResultVO();
        result.setQuestion(question);
        result.setQueryMode(AiQueryMode.HYBRID_PIPELINE.name());
        result.setFinalQueryMode(route.getQueryMode());
        result.setQueryType(queryType);
        result.setRouteReason(route.getRouteReason());
        ...
        result.getMessages().add("当前执行链路：设备解析 -> 运行时物模型语义 -> Hybrid 多源执行器。");
        ...
        if (QUERY_TYPE_CURRENT.equals(queryType)) {
            handleCurrentQuery(result, device, deviceLog, runtimeField, window);
    ...
    private void handleCurrentQuery(AiHybridQueryResultVO result, Device device, DeviceLog deviceLog,
                                    AiSemanticFieldVO runtimeField, QueryWindow window) {
        ValueItem valueItem = itslValueCache.getCacheIdentifier(device.getProductId(), device.getSerialNumber(), result.getIdentifier());
        if (valueItem != null) {
            result.setFinalQueryMode(AiQueryMode.REDIS_VALUE.name());
            result.setCurrentValue(valueItem.getValue());
            result.setCurrentTime(formatDate(valueItem.getTs()));
            result.getMessages().add("当前值优先命中 Redis 实时缓存。");
            result.setSummary(buildCurrentSummary(result));
            return;
        }

        if (isFunctionModel(runtimeField, device.getProductId(), result.getIdentifier())) {
            List<HistoryModel> historyList = loadFunctionHistory(device, deviceLog.getIdentify(), window);
            HistoryModel latest = firstHistoryModel(historyList);
            if (latest == null) {
                throw new ServiceException(message("ai.hybrid.query.current.function.value.not.found"));
            }
            result.setFallbackUsed(Boolean.TRUE);
            result.setFinalQueryMode(AiQueryMode.TSDB_QUERY.name());
            result.setCurrentValue(latest.getValue());
            result.setCurrentTime(formatDate(latest.getTime()));
            result.getMessages().add("Redis 未命中，已自动回退到 FunctionLog 最近一次记录。");
            result.setSummary(buildCurrentSummary(result));
            return;
        }
        ...
        if (lastReport == null) {
            throw new ServiceException(message("ai.hybrid.query.current.value.not.found"));
        }
        result.setFallbackUsed(Boolean.TRUE);
        result.setFinalQueryMode(AiQueryMode.TSDB_QUERY.name());
        result.setCurrentValue(lastReport.getLogValue());
        result.setCurrentTime(resolveLogTime(lastReport));
        result.getMessages().add("Redis 未命中，已自动回退到 TSDB 最近一次上报值。");
        result.setSummary(buildCurrentSummary(result));
    }

    private void handleHistoryQuery(AiHybridQueryResultVO result, DeviceLog deviceLog) {
        List<HistoryBo> historyList = logService.selectHistorySingleBo(deviceLog);
        if (historyList == null || historyList.isEmpty()) {
            throw new ServiceException(message("ai.hybrid.query.tsdb.history.not.found"));
        }
        ...
        for (HistoryBo historyBo : historyList) {
            ...
            point.setIdentifier(StringUtils.isNotBlank(historyBo.getIdentify()) ? historyBo.getIdentify() : result.getIdentifier());
            points.add(point);
        }
        ...
        result.getMessages().add("历史值通过 TSDB 查询完成。");
        result.setSummary(buildHistorySummary(result));
    }

    private void handleFunctionHistoryQuery(AiHybridQueryResultVO result, Device device, String identifier, QueryWindow window) {
        List<HistoryModel> historyList = loadFunctionHistory(device, identifier, window);
        if (historyList == null || historyList.isEmpty()) {
            throw new ServiceException(message("ai.hybrid.query.function.history.not.found"));
        }
        List<AiTsdbHistoryPointVO> points = new ArrayList<>();
        for (HistoryModel historyModel : historyList) {
            ...
            point.setIdentifier(StringUtils.isNotBlank(historyModel.getIdentify()) ? historyModel.getIdentify() : result.getIdentifier());
            points.add(point);
        }
        points.sort(Comparator.comparing(AiTsdbHistoryPointVO::getTime, Comparator.nullsLast(String::compareTo)));
        result.setFinalQueryMode(AiQueryMode.TSDB_QUERY.name());
        result.setHistoryPoints(points);
        result.getMessages().add("历史值通过 FunctionLog 查询完成。");
        result.setSummary(buildHistorySummary(result));
    }

    private void handleAggregateQuery(AiHybridQueryResultVO result, String question, DeviceLog deviceLog) {
        ...
        if (valueList == null || valueList.isEmpty()) {
            throw new ServiceException(message("ai.hybrid.query.tsdb.stat.samples.not.found"));
        }
        result.setFinalQueryMode(AiQueryMode.TSDB_QUERY.name());
        ...
        result.getMessages().add("统计值通过 TSDB 聚合样本计算完成。");
        result.setSummary(buildAggregateSummary(result));
    }

    private void handleFunctionAggregateQuery(AiHybridQueryResultVO result, String question, Device device,
                                              String identifier, QueryWindow window) {
        Integer operation = resolveStatisticOperation(question);
        List<HistoryModel> historyList = loadFunctionHistory(device, identifier, window);
        List<String> valueList = collectFunctionHistoryValues(historyList);
        if (valueList.isEmpty()) {
            throw new ServiceException(message("ai.hybrid.query.function.stat.samples.not.found"));
        }
        result.setFinalQueryMode(AiQueryMode.TSDB_QUERY.name());
        result.setStatisticOperation(resolveOperationName(operation));
        result.setStatisticSamples(new ArrayList<>(valueList));
        result.setStatisticValue(calculateStatisticValue(valueList, operation));
        result.getMessages().add("统计值通过 FunctionLog 历史样本计算完成。");
        result.setSummary(buildAggregateSummary(result));
    }
    ...
    private AiSemanticFieldVO resolveRuntimeField(String question, AiSemanticContextVO context) {
        ...
        return AiRuntimeFieldSelectionSupport.resolveRuntimeField(question, runtimeFields,
                "未命中可用的运行时物模型语义，无法执行 Hybrid 问数。", "Hybrid 问数");
    }
    ...
    private String resolveQueryType(String question) {
        if (containsAny(question, "当前", "实时", "现在", "此刻", "最新", "最近一次", "最后一次")) {
            return QUERY_TYPE_CURRENT;
        }
    ...
    private String buildCurrentSummary(AiHybridQueryResultVO result) {
        ...
        builder.append(" 当前值为 ").append(defaultValue(result.getCurrentValue()));
        if (StringUtils.isNotBlank(result.getUnit())) {
            builder.append(result.getUnit());
        }
        if (StringUtils.isNotBlank(result.getCurrentTime())) {
            builder.append("，时间 ").append(result.getCurrentTime());
        }
        return builder.toString();
    }

    private String buildAggregateSummary(AiHybridQueryResultVO result) {
        StringBuilder builder = new StringBuilder();
        builder.append("设备 ").append(result.getSerialNumber()).append(" 的 ");
        builder.append(StringUtils.isNotBlank(result.getSemanticName()) ? result.getSemanticName() : result.getIdentifier());
    ...
    private String buildHistorySummary(AiHybridQueryResultVO result) {
        StringBuilder builder = new StringBuilder();
        builder.append("设备 ").append(result.getSerialNumber()).append(" 的 ");
        builder.append(StringUtils.isNotBlank(result.getSemanticName()) ? result.getSemanticName() : result.getIdentifier());
        builder.append(" 在 ").append(defaultValue(result.getTimeWindowLabel()));
    ...
    private boolean isHybridCompatibleRoute(String queryMode) {
        return AiQueryMode.HYBRID_PIPELINE.name().equals(queryMode)
                || AiQueryMode.REDIS_VALUE.name().equals(queryMode)
                || AiQueryMode.TSDB_QUERY.name().equals(queryMode);
    }
