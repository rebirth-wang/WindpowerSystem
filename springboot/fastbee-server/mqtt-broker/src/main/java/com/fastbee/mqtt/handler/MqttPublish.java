...
public class MqttPublish implements MqttHandler {
    ...
    public void sendToMQ(MqttPublishMessage message, String clientId) {
        ...
        if (!(topicName.contains(TopicType.HTTP_UPGRADE_REPLY.getTopicSuffix()) || topicName.contains(TopicType.FETCH_UPGRADE_REPLY.getTopicSuffix()))) {
            ...
            byte[] source1 = s.getBytes(StandardCharsets.UTF_8);
            if (source.length != source1.length) {
                source = ByteBufUtil.hexDump(Unpooled.wrappedBuffer(message.content())).getBytes();
            }
//            MsgContext context = scriptService.processRuleScript(clientId, ScriptEventEnum.DEVICE_REPORT.getType(), topicName, new String(source));
...
//            }
        ...
        if (reportBo.getTopicName().contains("property")) {
            ...
            if (jsonObject == null) {
                log.warn("sendToMQ: status消息解析为null, clientId={}, topicName={}", clientId, topicName);
                return;
            }
            Integer statusVal = jsonObject.getInteger("status");
            if (statusVal == null) {
                log.warn("sendToMQ: status字段为null, clientId={}, topicName={}", clientId, topicName);
                return;
            }
            DeviceStatusBo bo = DeviceStatusBo.builder()
                    .serialNumber(topicsUtils.parseSerialNumber(reportBo.getTopicName()))
                    .status(DeviceStatus.convert(statusVal))
                    .build();
