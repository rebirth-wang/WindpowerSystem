...
import com.fastbee.isup.config.HikPlatformProperties;
import com.fastbee.isup.model.IsupDevInfo;
import com.fastbee.isup.model.tts.DataItem;
import com.fastbee.isup.model.tts.TtsRequest;
import com.fastbee.isup.service.DeviceCacheService;
import com.fastbee.isup.service.IMediaStreamService;
import com.fastbee.isup.util.WebFluxHttpUtil;
...
public class VoiceController {
     ...
     */
    @PostMapping("/tts")
    public Mono<AjaxResult> sendTtsVoice(
        ...
        try {
            ...
            return Mono.just(AjaxResult.error("服务器内部错误，无法创建上传目录"));
        }
        // 生成唯一文件名
        ...
        if (!deviceOpt.isPresent()) {
            return Mono.just(AjaxResult.error("设备ID不存在: " + deviceId));
        }
        ...
        return WebFluxHttpUtil.postAsync(ttsServerUrl, request, byte[].class)
                .flatMap(audioData -> {
                    ...
                    return Mono.fromCallable(() -> {
                        ...
                        return AjaxResult.success(pcmTargetPath.toAbsolutePath().toString());
                    });
                })
                .doOnError(error -> {
                    // 发生错误时清理临时文件
     ...
     */
    @PostMapping("/upload")
    public Mono<AjaxResult> uploadVoice(
            @PathVariable String deviceId,
        ...
        return filePart.transferTo(targetPath)
                .then(Mono.fromCallable(() -> {
                    ...
                    return AjaxResult.success(g711uFileFullPath);
                }))
                .doOnError(error -> {
                    // 发生错误时清理临时文件
                    try {
