...
public class PreviewStreamHandler implements PREVIEW_DATA_CB {
    ...
    public PreviewStreamHandler(VideoSessionInfo info) {
        this.info = info;
    }
    ...
    public void invoke(int iPreviewHandle, NET_EHOME_PREVIEW_CB_MSG pPreviewCBMsg, Pointer pUserData) {
        ...
        // 获取或创建该句柄对应的连接
        RtpConnection connection = connectionMap.computeIfAbsent(iPreviewHandle, handle -> createRtpConnection(handle, mediaServerIp, rtpPort, sessionId));
        ...
        if (dataStream != null && dataStream.length > 0) {
            if (pPreviewCBMsg.byDataType == 2) {
                processRtpData(iPreviewHandle, pPreviewCBMsg, connection);
            }
        }
    }
    ...
    private RtpConnection createRtpConnection(int handle, String ip, int port, int sessionId) {
        ...
        try {
            ...
            conn.rtpOutputStream = conn.tcpSocket.getOutputStream();
    ...
    private void processRtpData(int handle, NET_EHOME_PREVIEW_CB_MSG msg, RtpConnection connection) {
        int dwBufSize = msg.dwDataLen;
        Pointer pBuffer = msg.pRecvdata;
