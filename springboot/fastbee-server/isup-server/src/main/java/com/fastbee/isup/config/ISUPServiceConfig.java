...
import com.fastbee.isup.sdk.service.*;
import com.fastbee.isup.sdk.structure.BYTE_ARRAY;
import com.fastbee.isup.util.OsSelect;
...
public class ISUPServiceConfig {
    ...
    public IHikISUPAlarm hikISUPAlarm() {
        ...
        synchronized (IHikISUPAlarm.class) {
            ...
            try {
                ...
                if (OsSelect.isWindows())
                    //win系统加载库路径(路径不要带中文)
                    strDllPath = System.getProperty("user.dir") + "\\lib\\windows\\HCISUPAlarm.dll";
                else if (OsSelect.isLinux())
                    //Linux系统加载库路径(路径不要带中文)
                    strDllPath = System.getProperty("user.dir") + "lib/linux/libHCISUPAlarm.so";
                hikISUPAlarm = Native.loadLibrary(strDllPath, IHikISUPAlarm.class);
            } catch (Exception ex) {
        ...
        if (OsSelect.isWindows()) {
            ...
            String strPathCrypto = System.getProperty("user.dir") + "\\lib\\windows\\libeay32.dll"; //Linux版本是libcrypto.so库文件的路径
            ...
            String strPathSsl = System.getProperty("user.dir") + "\\lib\\windows\\ssleay32.dll";    //Linux版本是libssl.so库文件的路径
            ...
            String strPathCom = System.getProperty("user.dir") + "\\lib\\windows\\HCAapSDKCom";        //只支持绝对路径，建议使用英文路径
            ...
            BYTE_ARRAY ptrByteArrayCrypto = new BYTE_ARRAY(256);
            String strPathCrypto = System.getProperty("user.dir") + "lib/linux/libcrypto.so"; //Linux版本是libcrypto.so库文件的路径
            System.arraycopy(strPathCrypto.getBytes(), 0, ptrByteArrayCrypto.byValue, 0, strPathCrypto.length());
            ptrByteArrayCrypto.write();
            hikISUPAlarm.NET_EALARM_SetSDKInitCfg(0, ptrByteArrayCrypto.getPointer());
            //设置libssl.so所在路径
            BYTE_ARRAY ptrByteArraySsl = new BYTE_ARRAY(256);
            String strPathSsl = System.getProperty("user.dir") + "/lib/linux/libssl.so";    //Linux版本是libssl.so库文件的路径
            System.arraycopy(strPathSsl.getBytes(), 0, ptrByteArraySsl.byValue, 0, strPathSsl.length());
            ptrByteArraySsl.write();
            hikISUPAlarm.NET_EALARM_SetSDKInitCfg(1, ptrByteArraySsl.getPointer());
            //报警服务初始化
            ...
            //设置HCAapSDKCom组件库文件夹所在路径
            BYTE_ARRAY ptrByteArrayCom = new BYTE_ARRAY(256);
            String strPathCom = System.getProperty("user.dir") + "lib/linux/HCAapSDKCom/";        //只支持绝对路径，建议使用英文路径
            System.arraycopy(strPathCom.getBytes(), 0, ptrByteArrayCom.byValue, 0, strPathCom.length());
            ptrByteArrayCom.write();
            hikISUPAlarm.NET_EALARM_SetSDKLocalCfg(5, ptrByteArrayCom.getPointer());
        }
        hikISUPAlarm.NET_EALARM_SetLogToFile(3, "/isup/log", false);
        return hikISUPAlarm;
    ...
    public IHikISUPStorage hikISUPStorage() {
        ...
        synchronized (IHikISUPStorage.class) {
            ...
            try {
                ...
                if (OsSelect.isWindows())
                    //win系统加载库路径(路径不要带中文)
                    strDllPath = System.getProperty("user.dir") + "\\lib\\windows\\HCISUPSS.dll";
                else if (OsSelect.isLinux())
                    //Linux系统加载库路径(路径不要带中文)
                    strDllPath = System.getProperty("user.dir") + "lib/linux/libHCISUPSS.so";
                hikISUPStorage = Native.loadLibrary(strDllPath, IHikISUPStorage.class);
            } catch (Exception ex) {
        ...
        if (OsSelect.isWindows()) {
            ...
            //设置sqlite3库的路径
            String strPathSqlite = System.getProperty("user.dir") + "\\lib\\windows\\sqlite3.dll";
            ...
            String strPathsplite = System.getProperty("user.dir") + "lib/linux/libsqlite3.so";    //Linux版本是libsqlite3.so库文件的路径
        ...
        //启用SDK写日志
        //hikISUPStorage.NET_ESS_SetLogToFile(3, "/isup/log", false);
        hikISUPStorage.NET_ESS_SetLogToFile(3, System.getProperty("user.dir") + "/container/StorageLog", false);
        return hikISUPStorage;
    }
    ...
    public HCISUPCMS ihcisupcms() {
        ...
        synchronized (HCISUPCMS.class) {
            ...
            try {
                if (OsSelect.isWindows())
                    //win系统加载库路径(路径不要带中文)
                    strDllPath = System.getProperty("user.dir") + "\\lib\\windows\\HCISUPCMS.dll";
                else if (OsSelect.isLinux())
                    //Linux系统加载库路径(路径不要带中文)
                    strDllPath = System.getProperty("user.dir") + "lib/linux/libHCISUPCMS.so";
        ...
        hcisupcms.NET_ECMS_SetLogToFile(3, "/isup/log", false);
        return hcisupcms;
    }
    ...
    public IHikISUPStream hikISUPStream() {
        ...
        synchronized (IHikISUPStream.class) {
            ...
            try {
                if (OsSelect.isWindows())
                    strDllPath = System.getProperty("user.dir") + "\\lib\\windows\\HCISUPStream.dll";
                else if (OsSelect.isLinux())
                    strDllPath = System.getProperty("user.dir") + "lib/linux/libHCISUPStream.so";
        ...
        hikISUPStream.NET_ESTREAM_SetLogToFile(3, "/isup/log", false);
    ...
    public IHikNet hikNet() {
        ...
        synchronized (IHikISUPStream.class) {
            ...
            try {
                if (OsSelect.isWindows())
                    strDllPath = System.getProperty("user.dir") + "\\lib\\windows\\netsdk\\HCNetSDK.dll";
                else if (OsSelect.isLinux())
                    strDllPath = System.getProperty("user.dir") + "lib/linux/netsdk/libhcnetsdk.so";
