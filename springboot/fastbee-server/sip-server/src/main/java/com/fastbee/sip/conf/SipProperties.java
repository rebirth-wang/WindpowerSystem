...
public class SipProperties {
    public static Properties getProperties(String name, boolean openLog) {
        return getProperties(name, openLog, null);
    }

    /**
     * @param name     栈名称
     * @param openLog  是否开启日志
     * @param serverId 集群实例标识，非空时附加到栈名称以保证唯一性
     */
    public static Properties getProperties(String name, boolean openLog, String serverId) {
        Properties properties = new Properties();
        String stackName = (serverId != null && !serverId.isEmpty()) ? name + "_" + serverId : name;
