package com.fastbee.sip.server;

import java.util.Date;

import javax.annotation.Nonnull;
import javax.annotation.Nullable;
import javax.sip.RequestEvent;

import com.fastbee.sip.domain.SipDevice;
import com.fastbee.sip.server.msg.DeviceControl;
import com.fastbee.sip.server.msg.SipMessage;

public interface MessageInvoker {

    /**
     * 发送订阅报警指令
     *
     * @param device             设备信息
     * @param startAlarmPriority 开始报警级别
     * @param endAlarmPriority   结束报警级别
     * @param expires            过期时间
     * @param alarmMethod        报警方式
     * @param startAlarmTime     开始报警时间
     * @param endAlarmTime       结束报警时间
     * @return void
     */
    void subscribe(@Nonnull SipDevice device,
                   int startAlarmPriority,
                   int endAlarmPriority,
                   int expires,
                   @Nonnull String alarmMethod,
                   @Nullable Date startAlarmTime,
                   @Nullable Date endAlarmTime);

    /**
     * 发送设备控制指令,{@link DeviceControl#getDeviceId()}为通道ID
     *
     * @param device  设备信息
     * @param command 控制指令
     */
    Boolean deviceControl(SipDevice device, DeviceControl command);

    String frontEndCmdString(int cmdCode, int parameter1, int parameter2, int combineCode2);
    void ptzCmd(SipDevice device, String channelId, int leftRight, int upDown, int inOut, int moveSpeed,
                int zoomSpeed);
    void frontEndCmd(SipDevice device, String channelId, int cmdCode, int parameter1, int parameter2, int combineCode2);

    void presetQuery(SipDevice device, String channelId);
    /**
     * 获取设备详细信息
     *
     * @param device 设备信息
     */
    void deviceInfoQuery(SipDevice device);

    /**
     * 获取通道列表
     *
     * @param device 设备信息
     * @return 通道列表
     */
    void catalogQuery(SipDevice device);

    void recordInfoQuery(SipDevice device, String sn, String channelId, Date start, Date end);

    SipMessage messageToObj(RequestEvent event);

    <T> T getExecResult(String key, long timeout);
}
