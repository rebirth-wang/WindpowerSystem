...
public abstract class ReqAbstractHandler {
    ...
    public SIPResponse responseSdpAck(SIPRequest request, String sdp, SipConfig sipConfig) throws SipException, InvalidArgumentException, ParseException {
        ContentTypeHeader contentTypeHeader = SipFactory.getInstance().createHeaderFactory().createContentTypeHeader("APPLICATION", "SDP");
        // 兼容国标中的使用编码@域名作为RequestURI的情况
        SipURI sipURI = (SipURI)request.getRequestURI();
        if (sipURI.getPort() == -1) {
            log.info("[参数不全] sipURI port为：-1");
            sipURI = SipFactory.getInstance().createAddressFactory().createSipURI(sipConfig.getServerSipid(),  sipConfig.getIp()+":"+sipConfig.getPort());
        }
    ...
    public SIPResponse responseAck(SIPRequest sipRequest, int statusCode, String msg, ResponseAckExtraParam responseAckExtraParam) throws SipException, InvalidArgumentException, ParseException {
        ...
        if (responseAckExtraParam != null) {
            if (responseAckExtraParam.getSipURI() != null && sipRequest.getMethod().equals(Request.INVITE)) {
                log.debug("responseAck SipURI: {}:{}", responseAckExtraParam.getSipURI().getHost(), responseAckExtraParam.getSipURI().getPort());
