        ...
        if (request.getMessageCode() == MessageCode.POST) {
            ...
            if (session != null) {
                ...
                if (request.getContentLength() > 0) {
                    Device device = JSON.parseObject(request.getContent().toString(StandardCharsets.UTF_8), Device.class);
                    device.setActiveTime(DateUtils.getNowDate());
                    device.setStatus(DeviceStatus.ONLINE.getType());
                    session.setDevice(device);
                    coapSessionService.storeSession(queryParams.get("token"), session);
                    //更新设备信息
