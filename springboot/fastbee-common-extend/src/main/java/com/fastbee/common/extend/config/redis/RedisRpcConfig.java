package com.fastbee.common.extend.config.redis;
...
import com.fastbee.common.extend.core.media.CommonCallback;
...
public class RedisRpcConfig implements MessageListener {
    ...
    public final static String REDIS_REQUEST_CHANNEL_KEY = "REDIS_REQUEST_CHANNEL_KEY";
    ...
    private void handlerResponse(RedisRpcResponse response) {
        if (sysSipConfig.getServerId().equals(response.getToId())) {
            return;
        }
    ...
    private void handlerRequest(RedisRpcRequest request) {
        try {
            if (sysSipConfig.getServerId().equals(request.getFromId())) {
                return;
            }
            ...
            // 没有携带目标ID的可以理解为哪个sip服务器有结果就哪个回复，携带目标ID，但是如果是不存在的uri则直接回复404
            if (sysSipConfig.getServerId().equals(request.getToId())) {
                if (method == null) {
                    // 回复404结果
