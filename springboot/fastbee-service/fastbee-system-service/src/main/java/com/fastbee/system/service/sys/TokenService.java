...
public class TokenService {
    ...
    public LoginUser getLoginUser(HttpServletRequest request) {
        ...
        if (StringUtils.isNotEmpty(token)) {
            try {
                ...
                LoginUser user = null;
                if (!redisCache.containsKey(userKey)) {
                    // 从数据库获取用户信息
                    SysClient sysClient = new SysClient();
                    sysClient.setEnable("1");
                    sysClient.setToken(Constants.TOKEN_PREFIX + token);
                    List<SysClientVO> list = sysClientService.selectSysClientList(sysClient);
                    if (!list.isEmpty() && StringUtils.isNotBlank(list.get(0).getRemark())) {
                        user = reCreateToken(SysClientConvert.INSTANCE.convertSysClient(list.get(0)));
                    } else {
                        return user;
                    }
                } else {
    ...
    public int addToken(SysUser user, SysClient sysClient) {
        ...
        sysClient.setRemark(loginUser.getUser().getUserName());
        return sysClientService.insertSysClient(sysClient);
    }

    public int delToken(Long[] ids) {
        int ret = 0;
        for (Long id : ids) {
            SysClient sysClient = sysClientService.selectSysClientById(id);
            Claims claims = parseToken(sysClient);
            if (claims != null) {
                String uuid = (String) claims.get(Constants.LOGIN_USER_KEY);
                String userKey = getTokenKey(uuid);
                if (redisCache.containsKey(userKey)) {
                    if (redisCache.deleteObject(userKey)) {
                        ret += sysClientService.deleteSysClientById(id);
                    }
                } else {
                    ret += sysClientService.deleteSysClientById(id);
                }
    ...
    public LoginUser reCreateToken(SysClient sysClient) {
        if (StringUtils.isNotEmpty(sysClient.getRemark())) {
            LoginUser loginUser = (LoginUser) userDetailsServiceImpl.loadUserByUsername(sysClient.getRemark());
            if (loginUser != null) {
                refreshToken(loginUser, Math.toIntExact(sysClient.getTimeout()));
                return loginUser;
            }
        }
        return null;
    ...
    public void refreshToken(LoginUser loginUser) {
        ...
        String userIdKey = getUserIdKey(loginUser.getUserId());
        SysClient sysClient = new SysClient();
        sysClient.setToken(Constants.TOKEN_PREFIX + loginUser.getRequestToken());
        List<SysClientVO> list = sysClientService.selectSysClientList(sysClient);
        if (!list.isEmpty()) {
            return;
        }
