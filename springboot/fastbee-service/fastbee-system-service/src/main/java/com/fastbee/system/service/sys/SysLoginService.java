...
//            }
//        }
        SysDept sysDept = this.validateDeptStatus(deptId);
        ...
        // 生成token
        return createTokenAndBindSaLogin(loginUser);
    }
     ...
     * @return {@link SysDept }
     */
    private SysDept validateDeptStatus(Long deptId) {
        if (null == deptId) {
            return null;
        }
        SysDept sysDept = sysDeptService.selectDeptById(deptId);
        if (Objects.isNull(sysDept)) {
            throw new ServiceException("你的账号所属机构不存在，请联系管理员！");
        }
        if (0 != sysDept.getStatus()) {
            throw new ServiceException("你的账号所属机构已被停用，请联系管理员！");
        }
        return sysDept;
    }
...
//        UserDetails userDetails=userDetailsServiceImpl.loadUserByUsername(username);
        ...
        SysDept sysDept = this.validateDeptStatus(user.getDeptId());
        // 重写验证方法
    ...
    public String ssoLogin(String username, String password, String language) {
        ...
        this.validateDeptStatus(user.getDeptId());
        // 重写验证方法
    ...
    private String createTokenAndBindSaLogin(LoginUser loginUser) {
        String token = tokenService.createToken(loginUser);
        authTokenFacade.bindLoginUserWithLegacyTimeout(loginUser, token);
        return token;
    }

    public Long getTokenExpireTime(String token) {
        LoginUser loginUser = tokenService.getLoginUserByToken(token);
        return loginUser == null ? null : loginUser.getExpireTime();
    }
}
