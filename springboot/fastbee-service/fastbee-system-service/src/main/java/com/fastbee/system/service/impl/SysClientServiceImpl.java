...
import com.fastbee.system.service.sys.UserDetailsServiceImpl;
...
public class SysClientServiceImpl extends ServiceImpl<SysClientMapper,SysClient> implements ISysClientService
{
    ...
    @PostConstruct
    public void initSysClient() {
        SysClient query = new SysClient();
        query.setEnable("1");
        LambdaQueryWrapper<SysClient> lqw = buildQueryWrapper(query);
        List<SysClient> list = baseMapper.selectList(lqw);
        for (SysClient client : list) {
            // 重建缓存
            SysUser user = userService.selectUserByUserName(client.getRemark());
            if (ObjectUtil.isNotNull(user)) {
                LoginUser loginUser = (LoginUser) userDetailsServiceImpl.createLoginUser(user);
                if (loginUser != null) {
                    Claims claims = Jwts.parser()
                            .setSigningKey(client.getClientSecret())
                            .parseClaimsJws(client.getToken().substring(Constants.TOKEN_PREFIX.length()))
                            .getBody();
                    // 解析对应的权限以及用户信息
                    String uuid = (String) claims.get(Constants.LOGIN_USER_KEY);
                    loginUser.setToken(uuid);
                    tkenService.refreshToken(loginUser,Math.toIntExact(client.getTimeout()));
                }
            }
        }
    }
    ...
    private LambdaQueryWrapper<SysClient> buildQueryWrapper(SysClient query) {
        ...
        lqw.eq(query.getTenantId() != null, SysClient::getTenantId, query.getTenantId());
    ...
    public int insertSysClient(SysClient sysClient)
    {
        ...
        if (null != user.getDeptId()) {
            sysClient.setTenantId(user.getDept().getDeptUserId());
        } else {
            sysClient.setTenantId(user.getUserId());
        }
        sysClient.setCreateTime(DateUtils.getNowDate());
        sysClient.setCreateBy(user.getUserName());
