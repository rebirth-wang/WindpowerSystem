...
public class TSLValueCacheImpl implements ITSLValueCache {
    ...
    private static final DateTimeFormatter CACHE_DATE_FORMATTER = new DateTimeFormatterBuilder()
    ...
    public List<ValueItem> getCacheDeviceStatus(Long productId, String deviceNumber) {
        ...
        if (map == null || map.size() == 0) {
            ...
            // 获取redis缓存的物模型值
            valueList = map.values().stream().map(this::parseValueItem).filter(Objects::nonNull)
...
//            }
    ...
    public String getCacheIdentifierValue(Long productId, String serialNumber, String identifier) {
        String key = RedisKeyBuilder.buildTSLVCacheKey(productId, serialNumber);
        ValueItem valueItem = parseValueItem(redisCache.getCacheMapValue(key, identifier));
    ...
    public ValueItem getCacheIdentifier(Long productId, String serialNumber, String identifier) {
        String key = RedisKeyBuilder.buildTSLVCacheKey(productId, serialNumber);
        return parseValueItem(redisCache.getCacheMapValue(key, identifier));
    }

    private ValueItem parseValueItem(String cacheValue) {
        if (cacheValue == null || cacheValue.isBlank()) {
            return null;
        }
        JSONObject jsonObject = JSONObject.parseObject(cacheValue);
        ValueItem valueItem = new ValueItem();
        valueItem.setId(jsonObject.getString("id"));
        valueItem.setValue(jsonObject.getString("value"));
        valueItem.setShadow(jsonObject.getString("shadow"));
        valueItem.setName(jsonObject.getString("name"));
        valueItem.setRemark(jsonObject.getString("remark"));
        valueItem.setTs(parseCacheDate(jsonObject.getString("ts")));
        return valueItem;
    }

    private Date parseCacheDate(String dateText) {
        if (dateText == null || dateText.isBlank()) {
            return null;
        }
        try {
            LocalDateTime localDateTime = LocalDateTime.parse(dateText, CACHE_DATE_FORMATTER);
