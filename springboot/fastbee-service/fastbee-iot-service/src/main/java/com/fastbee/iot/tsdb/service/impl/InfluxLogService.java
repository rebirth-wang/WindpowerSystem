...
public class InfluxLogService implements ILogService {
    ...
    private void setDeviceLog(List<DeviceLog> deviceLogList, FluxRecord record, DeviceLog log) {
        ...
        log.setLogType(((Number) Objects.requireNonNull(record.getValueByKey("logType"))).byteValue());
        ...
        log.setIsMonitor(((Number) Objects.requireNonNull(record.getValueByKey("isMonitor"))).byteValue());
        log.setMode(((Number) Objects.requireNonNull(record.getValueByKey("mode"))).byteValue());
    ...
    public Map<String, List<HistoryModel>> selectHistoryList(DeviceLog deviceLog) {
        ...
        return list.stream().collect(Collectors.groupingBy(HistoryModel::getIdentify));
    }
    ...
    public Page<HistoryModel> listhistoryGroupByCreateTime(DeviceLog deviceLog) {
        ...
        fluxQuery.append("|> sort(columns: [\"_time\"], desc: true) ");
        fluxQuery.append("|> keep(columns: [\"_time\", \"identify\", \"logValue\"]) ");
        ...
        Page<HistoryModel> page = new Page<>(deviceLog.getPageNum(), deviceLog.getPageSize());
    ...
    public List<String> selectStatsValue(DeviceLog deviceLog) {
        ...
        fluxQuery.append("|> keep(columns: [\"logValue\"]) ");

        List<FluxTable> tables = queryApi.query(fluxQuery.toString());

        List<String> statsValues = new ArrayList<>();
        for (FluxTable table : tables) {
            for (FluxRecord record : table.getRecords()) {
                statsValues.add(Objects.requireNonNull(record.getValueByKey("logValue")).toString());
            }
        }
        return statsValues;
    }
    ...
    public List<HistoryBo> selectHistorySingleBo(DeviceLog deviceLog) {
        ...
        queryBuilder.append("from(bucket: \"").append(influxConfig.getBucket()).append("\") ");
        // 处理时间范围
        if (deviceLog.getBeginTime() != null && !deviceLog.getBeginTime().isEmpty()
                && deviceLog.getEndTime() != null && !deviceLog.getEndTime().isEmpty()) {
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        ...
        queryBuilder.append("|> filter(fn: (r) => r._measurement == \"").append(influxConfig.getMeasurement()).append("\") ");
        ...
        if (deviceLog.getSerialNumber() != null && !deviceLog.getSerialNumber().isEmpty()) {
            queryBuilder.append("  |> filter(fn: (r) => r[\"serialNumber\"] == \"").append(deviceLog.getSerialNumber()).append("\")\n");
        }

        if (deviceLog.getIdentify() != null && !deviceLog.getIdentify().isEmpty()) {
            queryBuilder.append("  |> filter(fn: (r) => r[\"identify\"] == \"").append(deviceLog.getIdentify()).append("\")\n");
        }
        queryBuilder.append("|> group() ");
        queryBuilder.append("  |> sort(columns: [\"_time\"], desc: false)\n");
        queryBuilder.append("  |> map(fn: (r) => ({ logValue: r._value, _time: r._time, identify: r.identify }))");

        String query = queryBuilder.toString();

        // 执行查询
        List<FluxTable> tables = queryApi.query(query);
        List<HistoryBo> historyBos = new ArrayList<>();

        for (FluxTable table : tables) {
            for (FluxRecord record : table.getRecords()) {
                String logValue = Objects.requireNonNull(record.getValueByKey("logValue")).toString();
                Instant ts = record.getTime();
                String identify = Objects.requireNonNull(record.getValueByKey("identify")).toString();

                HistoryBo historyBo = new HistoryBo();
                historyBo.setValue(logValue);
                if (ts != null) {
                    historyBo.setTime(Date.from(ts));
                }
                historyBo.setIdentify(identify);
