...
public class GenTableServiceImpl implements IGenTableService {
    ...
    public Page<GenTable> selectGenTableList(GenTable genTable) {
        return genTableMapper.selectPage(new Page<>(genTable.getPageNum(), genTable.getPageSize()), this.buildGenTableQueryWrapper(genTable));
    }
        ...
        for (String template : templates) {
            if (!StringUtils.containsAny(template, "sql.vm", "api.js.vm", "index.vue.vm", "index-tree.vue.vm")) {
                ...
                try {
                    ...
                    throw new ServiceException(StringUtils.format(MessageUtils.message("genTable.template.rendering.fail"), table.getTableName()));
                }
            }
        }
    }
    ...
    public void synchDb(String tableName) {
        ...
        if (StringUtils.isNotEmpty(saveColumns)) {
            genTableColumnMapper.insertOrUpdateBatch(saveColumns);
        }
        List<GenTableColumn> delColumns = tableColumns.stream().filter(column -> !dbTableColumnNames.contains(column.getColumnName())).collect(Collectors.toList());
        if (StringUtils.isNotEmpty(delColumns)) {
            List<Long> ids = StringUtils.toList(delColumns, GenTableColumn::getColumnId);
