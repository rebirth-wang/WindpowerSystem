...
public class GoviewProjectController extends BaseController {
     ...
     * 查询项目列表
     */
    @PreAuthorize("@ss.hasPermi('goview:project:list')")
    @ApiOperation("查询项目列表")
    @GetMapping("/list")
     ...
     * 获取项目详细信息
     */
    @PreAuthorize("@ss.hasPermi('goview:project:query')")
    @ApiOperation("获取项目详细信息")
    @GetMapping(value = "/{id}")
    public AjaxResult getInfo(@PathVariable("id") String id) {
        GoviewProject goviewProject = new GoviewProject();
        goviewProject.setId(id);
        return AjaxResult.success(goviewProjectService.selectGoviewProjectById(goviewProject));
    }

    /**
     * 新增项目
     */
    @PreAuthorize("@ss.hasPermi('goview:project:add')")
    @ApiOperation("新增项目")
     ...
     * 修改项目
     */
    @PreAuthorize("@ss.hasPermi('goview:project:edit')")
    @ApiOperation("修改项目")
    @Log(title = "项目", businessType = BusinessType.UPDATE)
    @PutMapping
    public AjaxResult edit(@RequestBody GoviewProject goviewProject) {
        SecurityUtils.checkUserOperatePermission(goviewProject.getTenantId(), goviewProject.getCreateBy());
     ...
     * 删除项目
     */
    @PreAuthorize("@ss.hasPermi('goview:project:remove')")
    @ApiOperation("删除项目")
    @Log(title = "项目", businessType = BusinessType.DELETE)
	@DeleteMapping("/{ids}")
    public AjaxResult remove(@PathVariable String[] ids) {
        List<GoviewProject> goviewProjectList = goviewProjectService.listByIds(Arrays.asList(ids));
        for (GoviewProject goviewProject : goviewProjectList) {
            SecurityUtils.checkUserOperatePermission(goviewProject.getTenantId(), goviewProject.getCreateBy());
        }
