...
public class ProtocolLoadUtils {
    ...
    public static ArrayMap<RuntimeSchema> getActiveMap(Class typeClass) {
        return getActiveMap(CACHE, typeClass);
    }

    public static RuntimeSchema getActiveMap(Class typeClass, int version) {
        ArrayMap<RuntimeSchema> schemaMap = getActiveMap(CACHE, typeClass);
        if (schemaMap == null) return null;
        return schemaMap.getOrDefault(version);
    }

    public static ArrayMap<RuntimeSchema> getActiveMap(Map<String, ArrayMap<RuntimeSchema>> root, final Class typeClass) {
    ...
    private static Map<Integer, Set<BaseStructure>> findMultiVersionFields(Map<String, ArrayMap<RuntimeSchema>> root, List<Field> fs) {
    ...
    private static void fillField(Map<String, ArrayMap<RuntimeSchema>> root, Map<Integer, Set<BaseStructure>> multiVersionFields, Column column, Field field, int position) {
        ...
        if (BaseStructure != null) {
            ...
            ArrayMap<RuntimeSchema> modelMap = getActiveMap(root, ClassUtils.getGenericType(field));
            if (versions == ALL)
