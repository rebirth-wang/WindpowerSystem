...
public class ModbusRtuPakDecoder {
    ...
    public PakModbusRtu decode(ByteBuf in, ExplainUtils explain){
        ...
        verify(bytes);
        RuntimeSchema<PakModbusRtu> runtimeSchema = headerSchemaMap.get(0);
        PakModbusRtu message = new PakModbusRtu();
        message.setPayload(in);
        message.setVerified(false);
        runtimeSchema.mergeFrom(in,message,explain);
        log.info("=>解析:[{}]",message);
        return message;
    }
    ...
    public PakModbusRtu decodeMessage(ByteBuf in, int version){
        ...
        byte[] bytes = new byte[in.writerIndex()];
        copy.readBytes(bytes);
        RuntimeSchema<PakModbusRtu> runtimeSchema = headerSchemaMap.get(version);
        PakModbusRtu message = new PakModbusRtu();
        message.setPayload(in);
        message.setVerified(false);
        runtimeSchema.mergeFrom(in,message,null);
        log.info("=>解析:[{}]",message);
        return message;
    }
