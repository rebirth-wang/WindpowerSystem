...
public class JT808Message extends Message {

    @Column(length = 2, desc = "消息ID")
    protected int messageId;
    @Column(length = 2, desc = "消息体属性")
    protected int properties;
    @Column(length = 1, desc = "协议版本号", version = 1)
    protected int protocolVersion;
    @Column(length = 6, charset = "BCD", desc = "终端手机号", version = {-1, 0})
    @Column(length = 10, charset = "BCD", desc = "终端手机号", version = 1)
    protected String clientId;
    @Column(length = 2, desc = "流水号")
    protected int serialNo;
    @Column(length = 2, desc = "消息包总数")
    protected Integer packageTotal;
    @Column(length = 2, desc = "包序号")
    ...
    public JT808Message copyThat(JT808Message that) {
    ...
    public static int reflectMessageId(Class<?> clazz) {
        ...
        if (protocol != null && protocol.value().length > 0)
            return protocol.value()[0];
        return 0;
    }
