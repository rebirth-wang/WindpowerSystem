...
watch(cycleTotal, () => {
  if (radioValue.value === 2) emit('update', 'hour', cycleTotal.value);
});
watch(averageTotal, () => {
  if (radioValue.value === 3) emit('update', 'hour', averageTotal.value);
});
watch(checkboxString, () => {
  if (radioValue.value === 4) emit('update', 'hour', checkboxString.value);
});
watch(
  () => props.radioParent,
  (val) => {
