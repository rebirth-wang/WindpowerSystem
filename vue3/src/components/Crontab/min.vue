...
watch(cycleTotal, () => {
  if (radioValue.value === 2) emit('update', 'min', cycleTotal.value);
});
watch(averageTotal, () => {
  if (radioValue.value === 3) emit('update', 'min', averageTotal.value);
});
watch(checkboxString, () => {
  if (radioValue.value === 4) emit('update', 'min', checkboxString.value);
});
watch(
  () => props.radioParent,
  (val) => {
