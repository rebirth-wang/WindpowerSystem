...
import { defaultTheme, globalThemeJson } from '@vb/settings/chartThemes/index'
import { requestInterval, previewScaleType, requestIntervalUnit } from '@vb/settings/designSetting'
// 记录记录
...
} from '@vb/store/modules/chartHistoryStore/chartHistoryStore.d'
// 画布枚举
import { MenuEnum, SyncEnum } from '@vb/enums/editPageEnum'

import { getUUID, loadingStart, loadingFinish, loadingError, isString, isArray } from '@vb/utils'
  ...
  actions: {
    ...
    setCopy(isCut = false) {
      try {
        ...
        if (index !== -1) {
          ...
          ElMessage.success(
            isCut
              ? i18n.global.t('visualBigScreen.management-674210-709')
              : i18n.global.t('visualBigScreen.management-674210-710')
          )
          loadingFinish()
        }
      } catch (value) {
    ...
    setGroup(id?: string | string[], isHistory = true) {
      try {
        ...
        ElMessage.error(i18n.global.t('visualBigScreen.management-674210-711'))
        loadingFinish()
      }
    },
    // * 解除分组
    setUnGroup(ids?: string[], callBack?: (e: CreateComponentType[]) => void, isHistory = true) {
      try {
        ...
        ElMessage.error(i18n.global.t('visualBigScreen.management-674210-712'))
        loadingFinish()
      }
    },
    // * 锁定
    ...
    computedScale() {
      if (this.getEditCanvas.editLayoutDom) {
        ...
        ElMessage.warning(i18n.global.t('visualBigScreen.management-674210-688'))
      }
    },
    // * 监听缩放
