...
import type { RequestGlobalConfigType, RequestConfigType } from '@vb/store/modules/chartEditStore/chartEditStore.d';
  ...
  if (typeof target === 'string') {
    if (target.startsWith(prefix)) {
      ...
      try {
        ...
        ElMessage.error(i18n.global.t('visualBigScreen.management-674210-52'));
      }
      return result;
    } else {
      return target;
...
export const customizeHttp = (targetParams: RequestConfigType, globalParams: RequestGlobalConfigType) => {
  ...
  try {
    ...
    ElMessage.error(i18n.global.t('visualBigScreen.management-674210-1648'));
  }
};
