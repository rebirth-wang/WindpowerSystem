...
import { DragKeyEnum, MouseEventButton } from '@vb/enums/editPageEnum'
import { createComponent } from '@vb/packages'
import { ConfigType } from '@vb/packages/index.d'
import { CreateComponentType, CreateComponentGroupType, PickCreateComponentType } from '@vb/packages/index.d'
...
import { loadingStart, loadingFinish, loadingError, setComponentPosition, JSONParse } from '@vb/utils'
...
export const dragHandle = async (e: DragEvent) => {
  ...
  try {
    ...
    loadingError()
    ElMessage.warning(i18n.global.t('visualBigScreen.management-674210-713'))
  }
}
...
export const mousedownBoxSelect = (e: MouseEvent, item?: CreateComponentType | CreateComponentGroupType) => {
  if (e.which == 2) return
  if (getKeyboardActive().space) return
...
export const useMouseHandle = () => {
  ...
  const mousedownHandle = (e: MouseEvent, item: CreateComponentType | CreateComponentGroupType) => {
    ...
    onClickOutSide()
    // 按下左键 + CTRL
    if (e.buttons === MouseEventButton.LEFT && getKeyboardActive().ctrl) return
