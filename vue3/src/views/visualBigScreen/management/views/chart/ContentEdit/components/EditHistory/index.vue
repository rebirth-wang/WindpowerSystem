<template>
  <div class="go-flex-items-center">
    <n-popover class="edit-history-popover" :show-arrow="false" size="small" trigger="click" placement="top-start">
      <template #trigger>
        <n-button class="mr-10" secondary size="small" :disabled="options.length === 0">
          <span class="btn-text">{{ t('visualBigScreen.management-674210-75') }}</span>
        </n-button>
      </template>
    ...
    <n-tooltip trigger="hover">
      ...
      <span>{{ t('visualBigScreen.management-674210-76') }}{{ editHistoryMax }}{{ t('visualBigScreen.management-674210-77') }}</span>
    </n-tooltip>
  </div>
</template>
...
import { historyActionTypeName } from '@vb/store/modules/chartHistoryStore/chartHistoryDefine'
import { CreateComponentType } from '@vb/packages/index.d'
import { editHistoryMax } from '@vb/settings/designSetting'
