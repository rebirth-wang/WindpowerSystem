      ...
      <template #action>
        <n-space justify="space-between">
          <n-space v-if="targetDataRequest">
            <n-tag :bordered="false" type="primary">{{ `${t('visualBigScreen.management-674210-803')}:` }}</n-tag>
            <n-input
              ...
              :minlength="1"
              :maxlength="16"
              :placeholder="t('visualBigScreen.management-674210-804')"
            ></n-input>
          </n-space>
          <span v-else></span>
          <n-space>
            <n-button @click="closeHandle">{{ t('visualBigScreen.management-674210-111') }}</n-button>
...
const closeAndSendHandle = () => {
  if (!props.targetDataRequest?.dataPondName) {
    ElMessage.warning(t('visualBigScreen.management-674210-805'));
    inputInstRef.value?.focus();
    return;
  }
  goDialog({
    message: t('visualBigScreen.management-674210-806'),
    isMaskClosable: true,
