<template>
  <n-collapse-item :title="t('visualBigScreen.management-674210-664')" name="2">
    ...
    <n-card class="collapse-show-box">
      ...
      <div v-for="eventName in BaseEvent" :key="eventName">
        <p>
          ...
          <br />
          <span class="func-keyword">async {{ eventName }}</span>
          (mouseEvent,components) {
        </p>
        <p class="go-ml-4">
  ...
  <n-modal class="go-chart-data-monaco-editor" v-model:show="showModal" :mask-closable="false">
    <n-card :bordered="false" role="dialog" size="small" aria-modal="true" style="width: 1200px; height: 700px">
      <template #header>
        <n-space>
          <n-text>{{ t('visualBigScreen.management-674210-665') }}</n-text>
        </n-space>
      </template>
      ...
      <n-layout has-sider sider-placement="right">
        <n-layout style="height: 580px; padding-right: 20px">
          <n-tabs v-model:value="editTab" type="card" tab-style="min-width: 100px;">
            <!-- 提示 -->
            <template #suffix>
              <n-text class="tab-tip" type="warning">{{ t('visualBigScreen.management-674210-666') }}</n-text>
            </template>
            <n-tab-pane
              ...
              <n-scrollbar trigger="none" style="max-height: 505px">
                <n-collapse class="go-px-3" arrow-placement="right" :default-expanded-names="[1, 2]">
                  <n-collapse-item title="mouseEvent" :name="1">
                    <n-text depth="3">{{ t('visualBigScreen.management-674210-672') }}</n-text>
                  </n-collapse-item>
                </n-collapse>
              </n-scrollbar>
            </n-tab-pane>
      ...
      <template #action>
        <n-space justify="space-between">
          <div class="go-flex-items-center">
            ...
            <n-text class="go-ml-2" depth="2">{{ t('visualBigScreen.management-674210-673') }}</n-text>
          </div>
...
import { BaseEvent } from '@vb/enums/eventEnum';
...
const EventTypeName = {
  [BaseEvent.ON_CLICK]: t('visualBigScreen.management-674210-693'),
  [BaseEvent.ON_DBL_CLICK]: t('visualBigScreen.management-674210-694'),
  [BaseEvent.ON_MOUSE_ENTER]: t('visualBigScreen.management-674210-695'),
  [BaseEvent.ON_MOUSE_LEAVE]: t('visualBigScreen.management-674210-696'),
};
