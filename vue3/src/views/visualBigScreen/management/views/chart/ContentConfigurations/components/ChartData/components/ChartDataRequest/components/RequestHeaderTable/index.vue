<template>
  <n-table class="go-request-header-table-box" :single-line="false" size="small">
    <thead>
      <tr>
        <th></th>
        <th>{{ t('visualBigScreen.management-674210-889') }}</th>
        <th>{{ t('visualBigScreen.management-674210-890') }}</th>
        <th>{{ t('visualBigScreen.management-674210-1629') }}</th>
        <th>{{ t('visualBigScreen.management-674210-878') }}</th>
      </tr>
    </thead>
    <tbody>
      <tr v-for="(item, index) in tableArray.content" :key="index">
        ...
        <td>
          <n-button v-if="item.error" class="go-ml-2" text type="error">
            {{ t('visualBigScreen.management-674210-879') }}
          </n-button>
          <n-button v-else class="go-ml-2" text type="primary">
            {{ t('visualBigScreen.management-674210-880') }}
          </n-button>
        </td>
      </tr>
    </tbody>
...
import { RequestParamsObjType } from '@vb/enums/httpEnum'
