    ...
    <n-card :bordered="false" role="dialog" size="small" aria-modal="true" style="width: 900px; height: 650px">
      ...
      <div class="pond-content">
        ...
        <div v-else class="no-data go-flex-center">
          ...
          <n-text :depth="3">{{ t('visualBigScreen.management-674210-807') }}</n-text>
        </div>
        <!-- 左侧列表 -->
      ...
      <template #action>
        <n-space justify="space-between">
          <n-button type="info" secondary :disabled="!pondData" @click="openPond(true)">
            {{ t('visualBigScreen.management-674210-808') }}
            <template #icon>
              <n-icon>
                <pencil-icon />
              </n-icon>
          ...
          <div>
            ...
            <n-button type="primary" @click="closeAndSendHandle">{{
              t('visualBigScreen.management-674210-809')
            }}</n-button>
          </div>
        </n-space>
      </template>
...
import { requestConfig } from '@vb/packages/public/publicConfig'
...
const editSaveHandle = (newData: RequestDataPondItemType) => {
  try {
    ...
    if (targetIndex !== -1) {
      ...
      ElMessage.success(t('visualBigScreen.management-674210-810'))
    } else {
      ElMessage.error(t('visualBigScreen.management-674210-811'))
    }
  } catch (error) {
    ElMessage.error(t('visualBigScreen.management-674210-811'))
  }
...
const createSaveHandle = (newData: RequestDataPondItemType) => {
  try {
    if (editData.value) {
      ...
      ElMessage.success(t('visualBigScreen.management-674210-812'))
    } else {
      ElMessage.error(t('visualBigScreen.management-674210-813'))
    }
  } catch (error) {
    ElMessage.error(t('visualBigScreen.management-674210-813'))
  }
...
const deletePond = (targetData: RequestDataPondItemType) => {
  goDialog({
    message: t('visualBigScreen.management-674210-814'),
    isMaskClosable: true,
    ...
    onPositiveCallback: () => {
      ...
      if (targetIndex !== -1) {
        ...
        ElMessage.success(t('visualBigScreen.management-674210-48'))
      } else {
        ElMessage.error(t('visualBigScreen.management-674210-815'))
      }
    }
  })
}
