      ...
      >
        ...
        <div v-if="isShowTools(item)" class="list-tools go-transition" @click="deleteHandle(item, index)">
          <n-button text type="default" color="#ffffff">
            ...
            <span>{{ t('visualBigScreen.management-674210-105') }}</span>
          </n-button>
        </div>
      </div>
    </div>
...
import { MacOsControlBtn } from '@vb/components/Tips/MacOsControlBtn/index'
...
import { ChartModeEnum } from '@vb/store/modules/chartLayoutStore/chartLayoutStore.d'
...
import { componentInstall, loadingStart, loadingFinish, loadingError, JSONStringify, goDialog } from '@vb/utils'
import { DragKeyEnum } from '@vb/enums/editPageEnum'
import { createComponent } from '@vb/packages'
import { ConfigType, CreateComponentType, PackagesCategoryEnum } from '@vb/packages/index.d'
import { ChatCategoryEnum } from '@vb/packages/components/Photos/index.d'
...
const deleteHandle = (item: ConfigType, index: number) => {
  goDialog({
    message: t('visualBigScreen.management-674210-67'),
