      ...
      >
        <template #trigger>
          <n-input-group>
            <n-input
              size="small"
              :placeholder="t('visualBigScreen.management-674210-62')"
        ...
        <div class="search-list-box">
          <n-scrollbar style="max-height: 500px">
            <n-empty v-show="!searchRes.length" size="small" :description="t('visualBigScreen.management-674210-63')"></n-empty>
            <div
...
import { createComponent } from '@vb/packages'
import { ConfigType, CreateComponentType } from '@vb/packages/index.d'
import { themeColor } from '../../hooks/useLayout.hook'
import { useChartEditStore } from '@vb/store/modules/chartEditStore/chartEditStore'
import { ChartModeEnum, ChartLayoutStoreEnum } from '@vb/store/modules/chartLayoutStore/chartLayoutStore.d'
import { useChartLayoutStore } from '@vb/store/modules/chartLayoutStore/chartLayoutStore'
import { isString, addEventListener, removeEventListener } from '@vb/utils'
import { fetchConfigComponent, fetchChartComponent } from '@vb/packages/index'
import { componentInstall, loadingStart, loadingFinish, loadingError } from '@vb/utils'
import { ChartGlobImage } from '@vb/components/Pages/ChartGlobImage'
import { GoIconify } from '@vb/components/GoIconify'
...
const chartModeList = [
  { label: t('visualBigScreen.management-674210-64'), icon: AlbumsIcon, value: ChartModeEnum.SINGLE },
  { label: t('visualBigScreen.management-674210-65'), icon: GridIcon, value: ChartModeEnum.DOUBLE }
]
...
const selectChartHandle = async (item: ConfigType) => {
  ...
  try {
    ...
    loadingError()
    ElMessage.warning(t('visualBigScreen.management-674210-66'))
  }
}
