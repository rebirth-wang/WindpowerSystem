<template>
  <content-box
    ...
    :title="t('visualBigScreen.management-674210-540')"
    :depth="2"
    @back="backHandle"
  ...
  >
    ...
    <n-space v-if="reverseList.length === 0" justify="center">
      <n-text class="not-layer-text">{{ t('visualBigScreen.management-674210-648') }}</n-text>
    </n-space>
...
import { ChartLayoutStoreEnum, LayerModeEnum } from '@vb/store/modules/chartLayoutStore/chartLayoutStore.d'
...
import { MenuOptionsItemType } from '@vb/views/chart/hooks/useContextMenu.hook.d'
import { useContextMenu } from '@vb/views/chart/hooks/useContextMenu.hook'
import { MenuEnum, MouseEventButton } from '@vb/enums/editPageEnum'
import { getKeyboardActive } from '@vb/utils/runtimeContext'
...
const layerModeList = computed(() => [
  { label: t('visualBigScreen.management-674210-649'), icon: LaptopIcon, value: LayerModeEnum.THUMBNAIL },
  { label: t('visualBigScreen.management-674210-650'), icon: ListIcon, value: LayerModeEnum.TEXT }
])
...
const mousedownHandle = (e: MouseEvent, item: CreateComponentType) => {
  ...
  // 若此时按下了 CTRL, 表示多选
  const id = item.id
  if (e.buttons === MouseEventButton.LEFT && getKeyboardActive().ctrl) {
    // 若已选中，则去除
