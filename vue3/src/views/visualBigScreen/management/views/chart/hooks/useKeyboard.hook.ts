...
import { WinKeyboard, MacKeyboard, MenuEnum } from '@vb/enums/editPageEnum'
import throttle from 'lodash/throttle'
import debounce from 'lodash/debounce'
import keymaster from 'keymaster'
import { setKeyboardDressShow } from '@vb/utils'
import { setKeyboardActive } from '@vb/utils/runtimeContext'

const defaultKeymasterFilter = keymaster.filter

const isEditableTarget = (target: EventTarget | null) => {
  const el = target as HTMLElement | null
  if (!el) return false
  return !!el.closest(
    '.monaco-editor, .go-editor-area, .go-chart-data-monaco-editor, input, textarea, [contenteditable="true"]'
  )
}
...
export const useAddKeyboard = () => {
  keymaster.filter = (event: KeyboardEvent) => {
    if (isEditableTarget(event.target) || isEditableTarget(document.activeElement)) {
      return false
    }
    return defaultKeymasterFilter(event)
  }
...
export const useRemoveKeyboard = () => {
  keymaster.filter = defaultKeymasterFilter
  document.onkeydown = () => {}
  document.onkeyup = () => {}
