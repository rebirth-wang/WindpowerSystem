...
import {
  ...
  ProjectInfoEnum,
  ChartEditStorage
} from '@vb/store/modules/chartEditStore/chartEditStore.d'
...
import { fetchChartComponent, fetchConfigComponent, createComponent } from '@vb/packages/index'
import { saveInterval } from '@vb/settings/designSetting'
import throttle from 'lodash/throttle'
// 接口状态
import { ResultEnum } from '@vb/enums/httpEnum'
// 接口
import { saveProjectApi, fetchProjectApi, uploadFile, updateProjectApi } from '@vb/api/path'
// 画布枚举
...
import { PublicGroupConfigClass } from '@vb/packages/public/publicConfig'
...
export const useSync = () => {
    ...
    projectData.componentList.forEach(async (e: CreateComponentType | CreateComponentGroupType) => {
      const intComponent = (target: CreateComponentType) => {
        ...
        if (!app.component(target.chartConfig.chartKey)) {
          ...
          app.component(target.chartConfig.conKey, fetchConfigComponent(target.chartConfig))
        }
      }
  ...
  // 确保项目数据存在的方法
  const ensureProjectData = async (): Promise<any> => {
    // 如果已经有数据，直接返回
    ...
    try {
      ...
      if (res && res.code === ResultEnum.SUCCESS && res.data) {
        ...
        console.warn('Failed to fetch project data:', res)
        return null
      }
    } catch (error) {
      console.error('Exception while fetching project data:', error)
      httpErrorHandle()
      return null
    }
  }
  ...
  // * 数据保存
  const dataSyncUpdate = throttle(async (projectDataParam: any = null, updateImg = true) => {
    // 确保有项目数据
    let effectiveProjectData = projectDataParam || projectData.value

    // 如果仍然没有数据，尝试获取
    if (!effectiveProjectData) {
      effectiveProjectData = await ensureProjectData()
    }
    if (!effectiveProjectData) {
      ElMessage.error(i18n.global.t('visualBigScreen.management-674210-714'))
      return
    }
    ...
    if (projectId === null || projectId === '') {
      ElMessage.error(i18n.global.t('visualBigScreen.management-674210-715'))
      return
    }
    chartEditStore.setEditCanvas(EditCanvasTypeEnum.SAVE_STATUS, SyncEnum.START)
      ...
      if (updateImg) {
        ...
        if (uploadRes && uploadRes.code === ResultEnum.SUCCESS) {
          await updateProjectApi({
            ...
            indexImage: uploadRes.data.url,
            createBy: effectiveProjectData.createBy,
            tenantId: effectiveProjectData.tenantId
          })
        }
      }
    } catch (error) {
