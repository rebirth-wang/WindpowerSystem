...
export const useScale = (localStorageInfo: ChartEditStorageType) => {
  ...
  const safeConfig = localStorageInfo?.editCanvasConfig || ({} as any)
  const width = ref(Number(safeConfig.width || 1920))
  const height = ref(Number(safeConfig.height || 1080))
  ...
  // 屏幕适配
  onMounted(() => {
    const previewScaleType = safeConfig.previewScaleType || PreviewScaleEnum.FIT
