<template>
  <div :class="`go-preview ${previewScaleType}`" @mousedown="dragCanvas">
...
import { getFilterStyle, setTitle } from '@vb/utils'
...
import { useInitVChartsTheme } from '@vb/hooks'

const chartEditStore = useChartEditStore()
const storageInfo = (await getSessionStorageInfo()) as ChartEditStorageType | { isRelease: false } | undefined

const projectName = chartEditStore.editCanvasConfig?.projectName || '大屏预览'
setTitle(`预览-${projectName}`)

const previewScaleType = computed(() => {
  return chartEditStore.editCanvasConfig?.previewScaleType || PreviewScaleEnum.FIT
})

const previewRefStyle = computed(() => {
  const config = chartEditStore.editCanvasConfig || ({} as any)
  return {
    overflow: 'hidden',
    ...getEditCanvasConfigStyle(config),
