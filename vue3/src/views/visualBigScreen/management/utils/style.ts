...
import { chartColors, chartColorsSearch, CustomColorsType } from '@vb/settings/chartThemes/index'
...
export const setHtmlTheme = (themeName?: string) => {
  ...
  if (themeName) {
    ...
    e.setAttribute('data-visual-big-screen-theme', themeName)
    return
  }
  if (!getActivePinia()) {
    // Fallback during very early app bootstrap before pinia is mounted.
    e.setAttribute('data-theme', 'light')
    e.setAttribute('data-visual-big-screen-theme', 'light')
    return
  }
  const designStore = useDesignStore()
  e.setAttribute('data-theme', designStore.themeName)
  e.setAttribute('data-visual-big-screen-theme', designStore.themeName)
}
