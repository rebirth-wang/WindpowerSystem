...
import { WinKeyboard } from '@vb/enums/editPageEnum';
import { RequestHttpIntervalEnum, RequestParamsObjType } from '@vb/enums/httpEnum';
import { CreateComponentType, CreateComponentGroupType } from '@vb/packages/index.d';
import { emitSpacePressHold } from '@vb/utils/runtimeContext';
...
export const renderLang = (lang: string, set = {}, tag = 'span') => {
  return () => h(tag, set, { default: () => i18n.global.t(lang) });
};
...
export const screenfullFn = (isFullscreen?: boolean, isEnabled?: boolean) => {
  ...
  ElMessage.warning(i18n.global.t('visualBigScreen.management-674210-1649'));
};
...
export const canvasCut = (html: HTMLElement | null, callback?: Function) => {
  if (!html) {
    ElMessage.error(i18n.global.t('visualBigScreen.management-674210-1650'));
  ...
  html2canvas(html, {
    ...
    ElMessage.success(i18n.global.t('visualBigScreen.management-674210-1651'));
...
export const newFunctionHandle = (
  ...
  try {
    ...
    return i18n.global.t('visualBigScreen.management-674210-1701');
  }
};
...
export const setKeyboardDressShow = (keyCode?: number) => {
  ...
  if (!dom) return;
  if (!keyCode) {
    emitSpacePressHold(false);
    dom.innerText = '';
    return;
  }
  if (keyCode && code.has(keyCode)) {
    if (keyCode == 32) emitSpacePressHold(true);
    dom.innerText = i18n.global.t('visualBigScreen.management-674210-1705', { key: code.get(keyCode) });
  }
};
...
export const addWindowUnload = () => {
  const leaveMessage = i18n.global.t('visualBigScreen.management-674210-1702');
  // 关闭网页出现离开提示
