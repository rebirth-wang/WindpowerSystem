<template>
  <n-button quaternary @click="modelShow = true" :title="t('visualBigScreen.management-674210-129')">
  ...
  <n-modal v-model:show="modelShow">
    <div class="go-system-color-setting">
      <n-space justify="space-between">
        <n-h3 class="title">{{ t('visualBigScreen.management-674210-130') }}</n-h3>
      ...
      <div class="model-content" ref="contentLeftRef">
        ...
        <div class="content-right">
          <div class="color-name-detail">
            <n-text v-if="appThemeDetail" class="color-name">{{ t(appThemeDetail.name) }}</n-text>
            <n-text v-else class="color-name">{{ t('visualBigScreen.management-674210-131') }}</n-text>
      ...
      <div class="model-footer">
        {{ t('visualBigScreen.management-674210-132') }}
...
import { AppThemeColorType } from '@vb/store/modules/designStore/designStore.d'
import { icon } from '@vb/plugins/icon'
import themeColorLogo from '@vb/assets/images/exception/theme-color.png'
import { loadAsyncComponent } from '@vb/utils'
import { useScroll } from '@vueuse/core'
import designColor from '@vb/settings/designColor.json'
    ...
    .content-right {
      ...
      .color-name-detail {
        ...
        .select-color {
          ...
          height: 20px;
          border-radius: 3px;
          background-image: url('@vb/assets/images/exception/texture.png');
        }
