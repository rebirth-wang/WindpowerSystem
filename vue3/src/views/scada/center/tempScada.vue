  ...
  <div class="scada-center-temp">
    ...
    <el-card>
      <el-row :gutter="10">
        ...
        <el-col :span="1.5">
          <el-button plain :icon="Edit" :disabled="single" @click="handleUpdate" v-hasPermi="['scada:center:query']">
            {{ $t('scada.center.tempScada.342311-2') }}
          </el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
        ...
        <el-row :gutter="20" v-loading="loading">
          ...
          >
            <el-checkbox-group v-model="ids" @change="checkboxChange">
              <el-card class="card-wrap" :body-style="{ padding: '0px', overflow: 'hidden' }" shadow="always">
                <div class="img-wrap">
                  <el-image
                ...
                <el-button-group class="tools-wrap">
                  ...
                  <el-button
                    ...
                    v-hasPermi="['scada:center:query']"
                  >
                    {{ $t('scada.center.tempScada.342311-3') }}
                  </el-button>
                  <span
                    style="width: 1px; margin: 0px 10px; font-size: 16px; color: #dcdfe6"
                  ...
                  <el-button
                    ...
                    v-hasPermi="['scada:center:share']"
                  >
                    {{ $t('scada.center.tempScada.342311-4') }}
                  </el-button>
                  <span
                    style="width: 1px; margin: 0px 10px; font-size: 16px; color: #dcdfe6"
                    v-hasPermi="['scada:center:remove']"
      ...
      <div v-if="showType == 'list'" style="margin-top: 16px">
        <el-table v-loading="loading" :data="centerList" :border="false" @selection-change="handleSelectionChange">
          ...
          />
          <el-table-column
            :label="$t('scada.center.indeieScada.373453-4')"
            align="center"
            prop="pageImage"
            min-width="100"
          >
          ...
          <el-table-column fixed="right" :label="$t('opation')" align="center" width="250">
            <template #default="scope">
              <el-button
                size="small"
                link
    ...
    <el-dialog :title="dialog.title" v-model="dialog.open" width="600px" append-to-body>
      <el-form ref="dialogForm" :model="dialog.form" :rules="dialog.rules" label-width="100px">
        ...
        <el-form-item :label="$t('scada.center.tempScada.342311-0')" prop="productId">
          <el-select
            v-model="dialog.form.productId"
            :placeholder="$t('scada.center.tempScada.342311-1')"
            clearable
            filterable
            style="width: 400px"
          >
...
import { Search, Refresh, Plus, Edit, Delete, Download, Tickets, Grid, SetUp, Share } from '@element-plus/icons-vue';
...
const dialog = reactive({
  ...
  rules: {
    ...
    productId: [{ required: true, message: proxy.$t('scada.center.tempScada.342311-1'), trigger: 'change' }],
  },
});
...
function getProductList() {
  listProduct({ pageNum: 1, pageSize: 9999 }).then((res: any) => {
    productList.value = res.rows.map((item: any) => ({ value: item.productId, label: item.productName }));
  });
}
...
function reset() {
  dialog.form = { type: 1, pageImage: '', pageName: '', productId: '', remark: '' };
...
function goToDetail(row: any) {
  const routeUrl = router.resolve({
    ...
    query: { id: row.id, guid: row.guid, productId: row.productId, type: 1 },
  });
...
onMounted(() => {
  getProductList();
  getList();
  if (getRouteQueryString(route.query, 'productId') && auth.hasPermi('scada:center:add')) {
    handleAdd();
    dialog.form.productId = getRouteQueryNumber(route.query, 'productId', 0);
  }
});
</script>
