      ...
      <el-row :gutter="10">
        ...
        <el-col :span="1.5">
          <el-button plain :icon="Edit" :disabled="single" @click="handleUpdate" v-hasPermi="['scada:center:query']">
            {{ $t('scada.center.indeieScada.373453-16') }}
          </el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
              ...
              <el-card class="card-wrap" :body-style="{ padding: '0px', overflow: 'hidden' }">
                ...
                <el-button-group class="tools-wrap">
                  ...
                  <el-button
                    ...
                    v-hasPermi="['scada:center:query']"
                  >
                    {{ $t('scada.center.indeieScada.373453-17') }}
                  </el-button>
                  <span
                    style="width: 1px; margin: 0px 10px; font-size: 16px; color: #dcdfe6"
                  ...
                  <el-button
                    ...
                    v-hasPermi="['scada:center:preview']"
                  >
                    {{ $t('scada.center.indeieScada.373453-18') }}
                  </el-button>
                  <span
                    style="width: 1px; margin: 0px 10px; font-size: 16px; color: #dcdfe6"
                  ...
                  <el-button
                    ...
                    v-hasPermi="['scada:center:share']"
                  >
                    {{ $t('scada.center.indeieScada.373453-19') }}
                  </el-button>
                  <span
                    style="width: 1px; margin: 0px 10px; font-size: 16px; color: #dcdfe6"
                    v-hasPermi="['scada:center:remove']"
...
const dialog = reactive({
  ...
  rules: {
    ...
    sceneId: [{ required: true, message: proxy.$t('scada.center.indeieScada.373453-12'), trigger: 'change' }],
  },
});
...
function reset() {
  dialog.form = { type: 3, pageImage: '', pageName: '', remark: '' };
...
function goToDetail(row: any) {
  const routeUrl = router.resolve({
    ...
    query: { id: row.id, guid: row.guid, type: 3 },
  });
...
onMounted(() => {
  getList();
  if (getRouteQueryString(route.query, 'isAdd') && auth.hasPermi('scada:center:add')) {
    handleAdd();
  }
});
</script>
